...
function TestPage() {
  const nftList = useNFTs("GjZsKpXbW8RdD6bHKxiEbmRBfKsieEmptVCLkSepMX5T");

  // Función para contar el número de repeticiones de un NFT
  const countRepetitions = (nft) => {
    const repetitions = nftList.filter(
      (item) => item.data.name === nft.data.name
    );
    return repetitions.length > 1 ? `x${repetitions.length}` : "";
  };

  // Filtrar la lista para mostrar solo una instancia de NFTs repetidos
  const uniqueNFTList = Array.from(
    new Set(nftList.map((nft) => nft.data.name))
  ).map((name) => {
